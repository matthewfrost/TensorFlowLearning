...
x = tf.random_uniform([], -1, 1, dtype="float")
y = tf.random_uniform([], -1, 1, dtype="float")
...
values = tf.range(1, 7) #1d tensor 0-6
out = tf.diag(values)
print(sess.run(out))
...
determintent = tf.matrix_determinant(x)
print(sess.run(determintent))
...
def f4(): return tf.reduce_mean(tf.square(x - y))
def f5(): return tf.reduce_sum(tf.abs(x - y))
out = tf.cond(average < 0, f4, f5)